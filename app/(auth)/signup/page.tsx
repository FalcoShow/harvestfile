// =============================================================================
// HarvestFile — Signup Page (Phase 12 v4)
// Compact form — fits in ~520px vertical height.
// All marketing content (headline, stats, OBBBA) lives in the layout's left panel.
// This page is ONLY the form + OAuth + footer links.
//
// Order: Logo icon → Heading → Google OAuth → "or" → 3 fields → CTA → links
// =============================================================================
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 hf-auth-stagger" style={{ '--stagger-index': 2 } as React.CSSProperties}>
        ...
        <span className="text-[11px] text-white/20">or sign up with email</span>
        <div className="flex-1 h-px bg-white/[0.07]" />
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="hf-auth-stagger" style={{ '--stagger-index': 3 } as React.CSSProperties}>
          <label htmlFor="signup_name" className="block text-[12px] font-semibold text-white/50 mb-1.5">Full name</label>
        ...
        <div className="hf-auth-stagger" style={{ '--stagger-index': 4 } as React.CSSProperties}>
          <label htmlFor="signup_email" className="block text-[12px] font-semibold text-white/50 mb-1.5">Email address</label>
          <input id="signup_email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className="hf-auth-input" placeholder="you@example.com" />
        </div>

        <div className="hf-auth-stagger" style={{ '--stagger-index': 5 } as React.CSSProperties}>
          <label htmlFor="signup_pw" className="block text-[12px] font-semibold text-white/50 mb-1.5">Password</label>
          <div className="relative">
            <input id="signup_pw" type={showPw ? 'text' : 'password'} required autoComplete="new-password" minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} className="hf-auth-input pr-10" placeholder="Min. 8 characters" />
          ...
          {password.length > 0 && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-[9px] font-bold min-w-[32px] text-right" style={{ color: strength.color }}>{strength.label}</span>
            </div>
          )}
        </div>

        {/* Gold CTA */}
        <div className="pt-1 hf-auth-stagger" style={{ '--stagger-index': 6 } as React.CSSProperties}>
      ...
      {/* ── Footer ─────────────────────────────────── */}
      <div className="text-center space-y-2 hf-auth-stagger" style={{ '--stagger-index': 7 } as React.CSSProperties}>
        <p className="text-[13px] text-white/40">
        ...
        <p className="text-[10px] text-white/15 leading-relaxed">
          ...
          <Link href="/terms" className="underline underline-offset-2 hover:text-white/25 transition-colors">Terms</Link>{' '}and{' '}
