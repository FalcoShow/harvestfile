// =============================================================================
// HarvestFile — Login Page (Phase 12 v4.2)
// Adds magic-link session handler for Founding Farmer welcome email flow.
// =============================================================================
...
function LoginForm() {
  ...
  // ── Magic-link handler: detect #access_token in URL, establish session, redirect
  useEffect(() => {
    ...
    (async () => {
      // Give Supabase client ~300ms to auto-detect + persist the session
      await new Promise((r) => setTimeout(r, 300));
      const { data, error: sessErr } = await supabase.auth.getSession();

      if (sessErr || !data.session) {
        setError('Magic link expired or invalid. Request a new one below.');
        setMagicLinkProcessing(false);
        window.history.replaceState(null, '', window.location.pathname + window.location.search);
        return;
      }

      // Clean the hash from the URL before redirecting
      window.history.replaceState(null, '', window.location.pathname + window.location.search);
      router.push(next);
      router.refresh();
    })();
  ...
  // Magic-link processing state — show spinner instead of login form
    ...
    return (
      <div className="space-y-6 text-center py-12">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <p className="text-[15px] text-white/60">Signing you in…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center hf-auth-stagger" style={{ '--stagger-index': 0 } as React.CSSProperties}>
        <div className="flex justify-center mb-5"><Logo size={38} /></div>
        <h2 className="text-[24px] font-extrabold text-white tracking-[-0.02em] mb-1.5">Welcome back</h2>
        <p className="text-[14px] text-white/40">Sign in to your Harvest<span className="text-[#C9A84C] font-semibold">File</span> account</p>
      </div>
      {error && (
      ...
      <div className="flex items-center gap-3 hf-auth-stagger" style={{ '--stagger-index': 2 } as React.CSSProperties}>
        <div className="flex-1 h-px bg-white/[0.07]" /><span className="text-[12px] text-white/20">or</span><div className="flex-1 h-px bg-white/[0.07]" />
      </div>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="hf-auth-stagger" style={{ '--stagger-index': 4 } as React.CSSProperties}>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <button type="button" onClick={handleReset} className="text-[12px] text-[#C9A84C]/50 hover:text-[#C9A84C] transition-colors font-medium">Forgot password?</button>
          </div>
          <div className="relative">
            ...
            <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/25 hover:text-white/50 transition-colors" aria-label={showPw ? 'Hide' : 'Show'}>
              {showPw ? (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" /><line x1="1" y1="1" x2="23" y2="23" /></svg>) : (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>)}
            </button>
          </div>
        </div>
      ...
      <div className="text-center hf-auth-stagger" style={{ '--stagger-index': 6 } as React.CSSProperties}>
        <p className="text-[14px] text-white/40">Don&apos;t have an account?{' '}<Link href="/signup" className="text-[#C9A84C] hover:text-[#E2C366] font-semibold transition-colors">Create one free</Link></p>
      </div>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="space-y-6"><div className="flex flex-col items-center gap-3"><div className="h-10 w-10 rounded-lg bg-white/[0.05] animate-pulse" /><div className="h-7 w-44 rounded-lg bg-white/[0.04] animate-pulse" /><div className="h-5 w-60 rounded bg-white/[0.03] animate-pulse" /></div><div className="space-y-4"><div className="h-[48px] rounded-xl bg-white/[0.04] animate-pulse" /><div className="h-[46px] rounded-xl bg-white/[0.04] animate-pulse" /><div className="h-[46px] rounded-xl bg-white/[0.04] animate-pulse" /><div className="h-[48px] rounded-xl bg-white/[0.04] animate-pulse" /></div></div>}><LoginForm /></Suspense>
  );
}