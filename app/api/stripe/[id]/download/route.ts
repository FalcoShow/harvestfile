...
export async function GET(
  ...
  try {
    ...
    const { data: report, error } = await supabase
      .from("reports")
      .select("*, farmers(name, county, state)")
      .eq("id", reportId)
    ...
    // === HEADER ===
    drawText("HarvestFile", {
      font: helveticaBold,
      size: 24,
      color: green,
    });
    y -= 4;
    drawText("AI-Powered ARC/PLC Optimization Report", {
      font: helveticaBold,
      size: 14,
      color: darkGray,
    });
    ...
    if (summary) {
      ...
      drawText(`Location: ${summary.county}, ${summary.state}`, {
        color: medGray,
      });
      drawText(`Total Base Acres: ${summary.total_base_acres}`, {
        color: medGray,
      });
    ...
    if (content.crop_analyses && content.crop_analyses.length > 0) {
      drawText("Crop-by-Crop Analysis", {
        font: helveticaBold,
        size: 14,
        color: darkGray,
      });
      ...
      for (const crop of content.crop_analyses) {
        ...
        drawText(`RECOMMENDATION: ${crop.recommendation}`, {
          font: helveticaBold,
          size: 11,
          color: green,
        });
    ...
    if (content.total_estimated_payments) {
      ...
      drawText(
        `Current Elections Est. Payment: $${formatNum(totals.current_elections)}`,
        { size: 10, color: medGray, x: margin + 10 }
      );
      drawText(
        `Optimized Elections Est. Payment: $${formatNum(totals.optimized_elections)}`,
        { size: 10, color: medGray, x: margin + 10 }
      );
      drawText(
    ...
    if (content.disclaimers) {
      addNewPageIfNeeded(60);
      drawText("Disclaimers", {
        font: helveticaBold,
        size: 10,
        color: medGray,
      });
      y -= 2;
      drawText(content.disclaimers, {
        size: 8,
    ...
    for (let i = 0; i < pages.length; i++) {
      const page = pages[i];
      page.drawText(
        `HarvestFile.com  |  Page ${i + 1} of ${pages.length}`,
        {
          x: margin,
          y: 25,
          size: 8,
    ...
    const farmerName = (report.farmers?.name || "farmer").replace(
      /[^a-zA-Z0-9]/g,
      "_"
    );
