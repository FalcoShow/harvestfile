...
export default function PricingPage() {
  ...
  const handleSubscribe = async (tier: string) => {
    if (tier === 'enterprise') {
      window.location.href = 'mailto:hello@harvestfile.com?subject=Enterprise%20Inquiry';
      return;
    }
    ...
    try {
      ...
      const res = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          priceType: billingCycle === 'annual' ? `${tier}_annual` : `${tier}_monthly`,
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-[#0a0f0d]">
      {/* ── Hero Section ── */}
      <div className="max-w-7xl mx-auto px-6 pt-28 pb-12 text-center">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {PRICING_TIERS.map((tier) => {
            // Calculate prices with clean integers (no floating point decimals)
            const monthlyPrice = tier.price ?? 0;
            const displayPrice = billingCycle === 'annual' && tier.price
              ? Math.round(monthlyPrice * 0.8)
              : monthlyPrice;
            const annualTotal = tier.price ? Math.round(monthlyPrice * 12 * 0.8) : 0;
            const annualSavings = tier.price ? Math.round(monthlyPrice * 12 * 0.2) : 0;
            ...
            {[
              {
                ...
                a: 'You get full access to every feature of the plan you choose — unlimited reports, saved farm operations, price alerts, everything. No credit card required until day 15.',
              },
              {
                q: 'Can I cancel anytime?',
                a: 'Yes. Cancel from your dashboard with one click. No contracts, no penalties.',
              },
              {
                q: 'What does the AI report include?',
                a: 'A personalized analysis of your operation covering ARC-CO vs PLC recommendations, estimated payment projections, price scenario modeling, every FSA form you need, and step-by-step enrollment instructions.',
              },
              {
                q: 'Is there still a free calculator?',
                a: 'Yes! The ARC/PLC calculator on our homepage is free forever — no login required. The paid plans add saved operations, unlimited AI reports, alerts, and professional features.',
              },
              {
                q: "I'm a Farm Credit lender. What's the Enterprise plan?",
                a: 'Enterprise gives you unlimited producer management, white-label client reports, API access, and a dedicated account manager. Contact us to discuss your portfolio.',
              },
              {
                q: 'Is my farm data secure?',
                a: 'Your data is encrypted in transit and at rest. We use Supabase with row-level security — every query is scoped to your organization. We never sell or share your data.',
              },
            ].map((faq, i) => (
              <details
                key={i}
