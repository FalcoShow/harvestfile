...
// HarvestFile — Surface 2 Deploy 2: Planting Readiness Scorecards
//
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-[rgba(27,67,50,0.30)] p-5 sm:p-6 h-full">
      ...
      <div className="space-y-3">
        {windows.map((w, i) => {
          ...
          return (
            <div key={i} className={`rounded-xl border ${confidenceBg(w.confidence)} p-3.5`}>
              {/* Crop header */}
              <div className="flex items-center justify-between mb-2.5">
                <div className="flex items-center gap-2">
                  <span className="text-base">{cropIcon(w.crop)}</span>
                  <span className="text-sm font-bold text-white/90">{w.crop}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-sm font-bold tabular-nums ${confidenceColor(w.confidence)}`}>
                    {w.confidence}%
                  </span>
                  <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${frost.bg} ${frost.text}`}>
                    {frost.label}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 text-[10px] text-white/30 mb-2">
                <span>Window: {w.optimal_start} – {w.optimal_end}</span>
              </div>
