// =============================================================================
// HarvestFile — Phase 32 Build 1.1: Founding Farmer Client Component
// FIXES: Clickable Bronze Steward link, linked tool boxes, emojis replaced
//        with premium SVG icons, branded success/footer elements
// =============================================================================
...
const TOOL_LINKS: Record<string, string> = {
  'ARC/PLC Calculator': '/check',
  'Payment Estimator': '/payments',
  'Base Acre Analyzer': '/fba',
  'SDRP Tool': '/sdrp',
  'USDA Calendar': '/calendar',
  ...
  'Breakeven': '/breakeven',
  'Cash Flow': '/cashflow',
  'Farm Score': '/farm-score',
  'Grain Marketing': '/grain',
  'AI Farm Advisor': '/advisor',
  'County Benchmarks': '/check',
};

// ── SVG Icons for Reward Tiers (no emojis) ───────────────────────────────────
function TierIcon({ tier, size = 32 }: { tier: string; size?: number }) {
  const iconSize = size * 0.45;
  const configs: Record<string, { bg: string; stroke: string }> = {
  ...
  const c = configs[tier] || configs.base;

  return (
    <div className="flex items-center justify-center rounded-full shrink-0" style={{ width: size, height: size, background: c.bg }}>
      {tier === 'base' && (
        <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /></svg>
      )}
      {tier === 'bronze' && (
        <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 15l-3 3h6l-3-3z" /><path d="M5 3h14l-2 8H7L5 3z" /></svg>
      )}
      {tier === 'silver' && (
        <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
      )}
      {tier === 'gold' && (
        <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="6" /><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11" /></svg>
      )}
      {tier === 'platinum' && (
        <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={c.stroke} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" /></svg>
      )}
    </div>
  );
}

// ── HarvestFile Logo SVG ─────────────────────────────────────────────────────
function HFLogo({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="32" height="32" rx="8" fill="#1B4332" />
      <path d="M8 10L16 6L24 10L16 14L8 10Z" fill="#C9A84C" />
      <path d="M8 10V18L16 22V14L8 10Z" fill="#2D6A4F" />
      <path d="M24 10V18L16 22V14L24 10Z" fill="#40916C" />
    </svg>
  );
}
...
function FoundingFarmerInner() {
  ...
  useEffect(() => {
    if (refCode) {
      fetch(`/api/founding-farmer?code=${encodeURIComponent(refCode)}`)
        .then((r) => r.json()).then(setReferrerInfo).catch(() => {});
    }
  }, [refCode]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0f0d] text-white overflow-x-hidden">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] backdrop-blur-sm p-6 sm:p-8">
            <div className="mb-6">
              ...
              <div className="h-3 rounded-full bg-white/[0.06] overflow-hidden">
                <div className="h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${Math.max(progressPercent, 1)}%`, background: 'linear-gradient(90deg, #1B4332 0%, #2D6A4F 40%, #C9A84C 100%)' }} />
              </div>
            </div>

            {result ? (
              <div className="text-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: 'linear-gradient(135deg, #1B4332, #2D6A4F)' }}><HFLogo size={32} /></div>
                ...
                <div className="flex items-center justify-center gap-3">
                  <a href={`sms:?body=${encodeURIComponent(`I just became Founding Farmer #${result.position} at HarvestFile — the most powerful farm decision platform ever built. Only ${stats.spots_remaining} spots left. Claim yours: ${result.share_url}`)}`} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#2D6A4F]/30 border border-[#2D6A4F]/40 text-sm font-semibold text-[#6FCF97] hover:bg-[#2D6A4F]/50 transition-colors">
                ...
                <button onClick={scrollToRewards} className="text-xs text-white/30 mt-6 hover:text-white/50 transition-colors cursor-pointer inline-block">
                  Refer 3 farmers to unlock <span className="text-[#CD7F32] font-semibold">Bronze Steward</span> rewards →
                </button>
              </div>
            ) : (
              <>
          ...
          {stats.recent_signups.length > 0 && (
            <div className="mt-4 space-y-1.5">
              {stats.recent_signups.slice(0, 3).map((signup) => (
                <div key={signup.position} className="flex items-center gap-3 rounded-lg bg-white/[0.02] px-4 py-2 animate-fadeIn">
                  <span className="w-2 h-2 rounded-full bg-emerald-500/80 animate-pulse" />
                  <span className="text-xs text-white/30"><span className="text-white/50 font-semibold">Founding Farmer #{signup.position}</span>{signup.state !== 'Unknown' && ` from ${signup.state}`}{' — '}{formatTimeAgo(signup.created_at)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* Reward Tiers — with scroll anchor */}
              ...
              <div key={tier.name} className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-5 hover:border-white/[0.12] transition-colors">
                <div className="flex items-center gap-4 sm:w-52 shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/30">{i === 0 ? 'Signup' : `${tier.referrals} referral${tier.referrals !== 1 ? 's' : ''}`}</p>
                  </div>
                </div>
                <div className="flex-1">
                  <div className="flex flex-wrap gap-2">
                    {tier.benefits.map((b) => (<span key={b} className="inline-flex items-center px-2.5 py-1 rounded-md bg-white/[0.04] text-[11px] text-white/50">{b}</span>))}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="relative z-10 px-4 py-20 border-t border-white/[0.04]">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {Object.entries(TOOL_LINKS).map(([name, href]) => (
              <Link key={name} href={href} className="rounded-lg bg-white/[0.03] border border-white/[0.05] px-3 py-2.5 text-xs text-white/40 font-medium hover:border-[#C9A84C]/30 hover:text-[#C9A84C]/80 hover:bg-[#C9A84C]/5 transition-all">{name}</Link>
            ))}
          </div>
          <div className="mt-10">
            <Link href="/check" className="inline-flex items-center gap-2 text-sm font-semibold text-[#C9A84C] hover:text-[#E2C366] transition-colors">
              Try the free ARC/PLC calculator right now<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
            </Link>
          </div>
        </div>
      </section>
            ...
            {[
              ...
              <details key={item.q} className="group rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                <summary className="flex items-center justify-between cursor-pointer px-5 py-4 text-sm font-semibold text-white/70 hover:text-white/90 transition-colors list-none [&::-webkit-details-marker]:hidden">
                  {item.q}
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 ml-4 text-white/20 transition-transform group-open:rotate-180"><polyline points="6 9 12 15 18 9" /></svg>
                </summary>
      ...
      <section className="relative z-10 px-4 py-20 border-t border-white/[0.04]">
        <div className="max-w-lg mx-auto text-center">
          ...
          {stats.is_open && !result && (
            <button onClick={() => emailInputRef.current?.focus()} className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl font-bold text-base bg-gradient-to-r from-[#C9A84C] to-[#E2C366] text-[#0a0f0d] hover:shadow-[0_0_30px_rgba(201,168,76,0.25)] transition-all">
              Claim Your Spot Now<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
            </button>
          )}
        </div>
      </section>
      ...
      <footer className="relative z-10 border-t border-white/[0.04] px-4 py-8">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-2">
            <HFLogo size={24} />
            <span className="text-xs text-white/30">Harvest<span className="text-[#C9A84C]">File</span> LLC — Tallmadge, Ohio</span>
          </Link>
...
export function FoundingFarmerClient() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#0a0f0d] flex items-center justify-center"><div className="w-8 h-8 border-2 border-[#C9A84C]/30 border-t-[#C9A84C] rounded-full animate-spin" /></div>}>
      <FoundingFarmerInner />
    </Suspense>
  );
}
...
function formatTimeAgo(dateStr: string): string {
  const now = Date.now(); const then = new Date(dateStr).getTime(); const diff = now - then;
